  ...
  <div v-loading="fetching" class="createPost-container">
    <el-form :model="formQuery" label-width="100px" class="demo-form-inline">
      ...
      <el-row class="topWrapper">
        <el-col :span="24" class="btnCls">
          <el-button type="primary" @click="onSubWord()">编辑单词</el-button>
    ...
    <el-dialog title="选择词根" :visible.sync="dialogVisible2" append-to-body width="60%">
      ...
      <el-table :data="data2" ref="crud" :header-cell-style="{
        ...
        <el-table-column label="选择" width="55">
          <template scope="scope">
            <el-radio :label="scope.$index" v-model="radio" @change.native="getCurrentRow2(scope.row)">&nbsp;</el-radio>
          </template>
        </el-table-column>
...
import { getWordList, getRootList, getCategoryTree, wordSave, getWordDetail } from '@/api/word'
...
export default {
  ...
  methods: {
    ...
    before () {
      ...
      getWordDetail(this.wordId).then((res) => {

        this.formQuery.name = res.data.name
        this.formQuery.example = res.data.example
        this.formQuery.fileUrl = res.data.fileUrl || ''
        this.formQuery.frequency = res.data.frequency
        this.formQuery.phonetic = res.data.phonetic
        this.formQuery.zhDesc = res.data.zhDesc
        this.formQuery.enDesc = res.data.enDesc
        this.formQuery.wordRoot = res.data.wordRoot || {}
        this.formQuery.categoryInfo = res.data.categoryInfo
        this.formQuery.similarIdList = res.data.similarIdList
        this.formQuery.antonymIdList = res.data.antonymIdList
        this.formQuery.id = res.data.id

        this.similarList = res.data.similarIdList
        this.antonymList = res.data.antonymIdList
        res.data.wordKindList.forEach((item) => {
          this.kindList.push(item.id)
          console.log(item)
        })
